...
public class InitialDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		...
		{
			if (configurationService.getConfiguration().getBoolean("isEPCEnabled"))
			{
				//LATAM related code
			...
			if (configurationService.getConfiguration().getBoolean("isEPCEMEAEnabled"))
			{

				//EMEA related code
				importCommonData(context, SAMPLE_DATA_IMPORT_FOLDER);

				importStoreInitialData(context, SAMPLE_DATA_IMPORT_FOLDER, PERSONAL_CARE_EMEA, PERSONAL_CARE_EMEA,
						Collections.singletonList(PERSONAL_CARE_EMEA));
				...
				// Send an event to notify any AddOns that the initial data import is complete
				getEventService().publishEvent(new SampleDataImportedEvent(context, Arrays.asList(emeaImportData)));
			}

			if (configurationService.getConfiguration().getBoolean("isEHPEnabled"))
			{
	...
	protected void importContentCatalog(final SystemSetupContext context, final String importDirectory, final String catalogName)
	{
		...
		if (catalogName.equalsIgnoreCase(PERSONAL_CARE_EMEA))
		{
			...
			final List<String> footerLinksList = Arrays.asList(footerLinksFromConfig.split(","));

			if (null != footerLinksList && !footerLinksList.isEmpty())
			{
				LOG.info("*********  Begin importing " + footerLinksList.size()
						+ " footer components sample data impex files from Content Catalog [" + catalogName + "] *********  ");

				for (final String footerLink : footerLinksList)
				{
					importImpexFile(context, importRoot + "/contentCatalogs/" + catalogName + "ContentCatalog/sampledata/"
							+ footerLink + "-sampledata.impex", true);
				}

				LOG.info("*********  Done importing " + footerLinksList.size()
						+ " footer components sample data impex files from Content Catalog [" + catalogName + "] *********  ");
			}
		}
