...
public class EnergizerMediaCSVProcessor extends AbstractJobPerformable<EnergizerCronJobModel>
{
	...
	public PerformResult perform(final EnergizerCronJobModel cronjob)
	{
		...
		try
		{
			...
			final String prefix = blobDirectoryForPersonalCareThumbNailPath.endsWith(fileSeperator)
					? blobDirectoryForPersonalCareThumbNailPath : blobDirectoryForPersonalCareThumbNailPath + fileSeperator;
			for (final BlobItem blobItemPCNP : blobContainerClient.listBlobsByHierarchy(prefix))
			{
				if (Boolean.TRUE.equals(blobItemPCNP.isPrefix()))
				{
					continue;
				}
