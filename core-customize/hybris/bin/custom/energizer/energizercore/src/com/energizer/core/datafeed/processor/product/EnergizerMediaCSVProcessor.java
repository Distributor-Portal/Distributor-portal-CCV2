...
BlobContainerClient blobContainerClient = null;
			...
			final String blobDirectoryForPersonalCareThumbNailPath = energizerMediaProcessor
					.getBlobDirectoryPersonalCareThumbNailPath(thumbnailPath);

ListBlobsOptions listBlobsOptions = new ListBlobsOptions();
			listBlobsOptions.setPrefix(blobDirectoryForPersonalCareThumbNailPath);
			for (final BlobItem blobItemPCNP : blobContainerClient.listBlobs(listBlobsOptions, null))
			{

				final String fullFilePath = blobItemPCNP.getName();
				final String fileName = StringUtils.substringAfterLast(fullFilePath, "/");
				BlobClient blob2;
				blob2 = blobContainerClient.getBlobClient(fullFilePath);
		...
		catch (final com.azure.storage.blob.models.BlobStorageException e1)
		{
			LOG.error("Blob storage error: " + e1.getMessage(), e1);
		}
		catch (final Exception e)
		{
			...
			final CatalogVersionModel catalogVersion, final String productMaterialId, final BlobContainerClient blobContainerClient)
	{
		...
		{
			LOG.error("Error getting blob input stream", e1);
		}
