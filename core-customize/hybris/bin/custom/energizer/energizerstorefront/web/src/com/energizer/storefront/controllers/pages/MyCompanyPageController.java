...
public class MyCompanyPageController extends AbstractSearchPageController
{
	...
	@ModelAttribute("b2bUnitCurrencyPreference")
	public List<SelectOption> getB2BUnitCurrencyPreference()
	{
		final String b2bUnit = b2bCommerceUnitFacade.getParentUnit().getUid();
		final EnergizerB2BUnitModel unitModel = (EnergizerB2BUnitModel) defaultB2BCommerceUnitService.getUnitForUid(b2bUnit);
		final CurrencyModel currencyModel = unitModel.getCurrencyPreference();
		final List<String> currencyList = new ArrayList<String>();
		currencyList.add(currencyModel.getIsocode());
		return populateSelectBoxForString(currencyList);
	}
	...
	protected String editCostCenterDetails(final String costCenterCode, final Model model) throws CMSItemNotFoundException
	{
		...
		breadcrumbs.add(new Breadcrumb(String.format(MANAGE_COSTCENTERS_EDIT_URL, costCenterCode),
				getMessageSource().getMessage("text.company.costCenter.editPage.breadcrumb", new Object[]
	...
	protected String createUser(final Model model) throws CMSItemNotFoundException, Exception
	{
		try
		{
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		try
		{
			...
			final EnergizerB2BCustomerModel b2bCustomerModel = (EnergizerB2BCustomerModel) userService
			...
			if (null != b2bCustomerModel)
			{
				...
				{
					if (permissionNotYetAssigned && (goups.getUid().equalsIgnoreCase("b2bcustomergroup")
							|| goups.getUid().equalsIgnoreCase("b2bapprovergroup")))
					{
	...
	public String editUser(final String user, final Model model) throws CMSItemNotFoundException
	{
		...
		breadcrumbs
				.add(new Breadcrumb(String.format("/my-company/organization-management/manage-users/edit?user=%s", urlEncode(user)),
