...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
	...
	private static final String SITE_EQUALS = "?site=";
			...
			throws IOException
	{
		...
		if (cmsSiteModel == null || StringUtils.contains(queryString, CLEAR_CMSSITE_PARAM))
		{
			...
			//cmsSiteModel = getContextInformationLoader().initializeSiteFromRequest(getSiteURL(httpRequest));

			// Added for Region Based Site Login, to determine the site using the currency from the request URL
			cmsSiteModel = getContextInformationLoader().initializeSiteFromRequest(getRegionBasedSiteURL(httpRequest));
		}
	...
	/**
	 * @param httpRequest
	 * @return String redirectURL, the URL to redirect
	 */
	public String getSiteURL(final HttpServletRequest httpRequest)
	{
		...
		String redirectURL = currentRequestURL;

		LOG.info("currentRequestURL before if condition ::: " + currentRequestURL);

		if (currentRequestURL.toLowerCase().contains("emea") || currentRequestURL.toLowerCase().contains("eur"))
		{
			redirectURL = PERSONALCARE_EMEA_WEBSITE;
			if (StringUtils.isBlank(queryString) || StringUtils.contains(queryString, "logout"))
			{
				redirectURL = redirectURL + LOGIN_URL + SITE_EQUALS + PERSONALCARE_EMEA + AND + CLEAR_CMSSITE_PARAM;
				LOG.info("The redirectURL contains emea. So, we are redirecting to EMEA Site " + redirectURL);
			}

		}
		else if ((!currentRequestURL.toLowerCase().contains("emea")) || (!currentRequestURL.toLowerCase().contains("eur")))
		{
			redirectURL = PERSONALCARE_WEBSITE;
			if (StringUtils.isBlank(queryString) || StringUtils.contains(queryString, "logout"))
			{
				redirectURL = redirectURL + LOGIN_URL + SITE_EQUALS + PERSONALCARE + AND + CLEAR_CMSSITE_PARAM;
				LOG.info("The redirectURL does not contains emea. So, we are redirecting to LATAM Site " + redirectURL);
			}
		}
		else
		{
			redirectURL = PERSONALCARE_WEBSITE + LOGIN_URL + SITE_EQUALS + PERSONALCARE + AND + CLEAR_CMSSITE_PARAM;
			LOG.info("The redirectURL does not contain any currency. So, we are redirecting to default LATAM Site " + redirectURL);
		}

		LOG.info("**********  Redirecting to " + redirectURL + " site ********** ");

		return redirectURL;
	}

	public String getRegionBasedSiteURL(final HttpServletRequest httpRequest)
	{

		LOG.info("**********  Entering getRegionBasedSiteURL method ********** ");
		...
		if (currentRequestURL.toLowerCase().contains("eur"))
		{
			redirectURL = PERSONALCARE_EMEA_WEBSITE + SITE_EQUALS + PERSONALCARE_EMEA;
			/*
			 * if (StringUtils.isBlank(queryString) || StringUtils.contains(queryString, "logout")) { redirectURL =
			 * redirectURL + SITE_EQUALS + PERSONALCARE_EMEA + AND + CLEAR_CMSSITE_PARAM;
			 * LOG.info("The redirectURL contains eur. So, we are redirecting to EMEA Site " + redirectURL); }
			 */
			LOG.info("The redirectURL contains eur. So, we are redirecting to EMEA Site " + redirectURL);
			if (StringUtils.contains(queryString, "logout"))
			{
				redirectURL = redirectURL + AND + CLEAR_CMSSITE_PARAM;
				LOG.info("The redirectURL contains eur and logout. So, we are redirecting to EMEA Site " + redirectURL);
			}

		}
		else if (currentRequestURL.toLowerCase().contains("usd"))
		{
			redirectURL = PERSONALCARE_WEBSITE + SITE_EQUALS + PERSONALCARE;
			/*
			 * if (StringUtils.isBlank(queryString) || StringUtils.contains(queryString, "logout")) { redirectURL =
			 * redirectURL + SITE_EQUALS + PERSONALCARE + AND + CLEAR_CMSSITE_PARAM;
			 * LOG.info("The redirectURL contains usd. So, we are redirecting to LATAM Site " + redirectURL); }
			 */
			LOG.info("The redirectURL contains usd. So, we are redirecting to LATAM Site " + redirectURL);
			if (StringUtils.contains(queryString, "logout"))
			{
				redirectURL = redirectURL + AND + CLEAR_CMSSITE_PARAM;
				LOG.info("The redirectURL contains usd and logout. So, we are redirecting to LATAM Site " + redirectURL);
			}

		}
		else
		{
			redirectURL = PERSONALCARE_WEBSITE + SITE_EQUALS + PERSONALCARE + AND + CLEAR_CMSSITE_PARAM;

			LOG.info("The redirectURL does not contain any currency. So, we are logging out of the site and redirecting to default LATAM Site "
					+ redirectURL);
		}
