...
public abstract class AbstractPageController extends AbstractController
{
	...
	public static final String JUST_UNLOCKED_ACCOUNT = "justunlockedaccount";
		...
		try
		{
			if (model.containsAttribute(CMS_PAGE_MODEL))
			{
				...
				if (page != null)
				{
					...
					{
						boolean isB2BViewergroup = false;
						final boolean isSalesRepUserLoggedIn = (boolean) getSessionService().getAttribute("isSalesRepUserLoggedIn");
						if (isSalesRepUserLoggedIn)
						{
							final EnergizerB2BCustomerModel selectedB2BCustomer = (EnergizerB2BCustomerModel) getSessionService()
									.getAttribute("selectedB2BCustomer");
							if (null != selectedB2BCustomer)
							{
								final Set<PrincipalGroupModel> groups = selectedB2BCustomer.getGroups();
								for (final PrincipalGroupModel principalGroupModel : groups)
								{
									if (principalGroupModel.getUid().equalsIgnoreCase("b2bviewergroup"))
									{
										isB2BViewergroup = true;
										break;
									}
								}
							}
						...
						model.addAttribute("isB2BViewergroup", isB2BViewergroup);
						//model.addAttribute("isSalesRepUser", isSalesRepUser);
					}
			...
			final String resolvedUrlPath) throws UnsupportedEncodingException
	{
		...
		final String requestURI = UriUtils.decode(request.getRequestURI(), "utf-8");
		final String decoded = UriUtils.decode(resolvedUrl, "utf-8");
		...
		/*
		 * catch (final URIException e) { throw new UnsupportedEncodingException(); }
		 */
	}
	...
	//Added Code changes for WeSell Implementation - START
	@ModelAttribute("isSalesRepUserLogin")
	public boolean isSalesRepUserLogin()
	{
		boolean isSalesRepUser = false;
		final String hybrisUserId = userService.getCurrentUser().getUid();
		if (null != hybrisUserId && !hybrisUserId.equalsIgnoreCase("anonymous"))
		{
			isSalesRepUser = (boolean) getSessionService().getAttribute("isSalesRepUserLoggedIn");

		}
		return isSalesRepUser;
	}

	@ModelAttribute("salesRepEmployee")
	public EnergizerB2BEmployeeData getSalesRepEmployeeData()
	{
		EnergizerB2BEmployeeData salesRepEmployeeData = null;
		try
		{
			final String hybrisUserId = userService.getCurrentUser().getUid();
			if (null != hybrisUserId && !hybrisUserId.equalsIgnoreCase("anonymous")
					&& ((boolean) getSessionService().getAttribute("isSalesRepUserLoggedIn")))
			{
				...
				if (null != salesRepEmployeeModel)
				{
					salesRepEmployeeData = energizerB2BEmployeeConverter.convert(salesRepEmployeeModel);
				}
			}
		}
		catch (final Exception e)
	...
	{
		return energizerProductService.getShippingPointLocation(shippingPointId);
	}
	...
	{
		return energizerProductService.getShippingPointName(shippingPointId);
	}
}
