...
public class StoreSessionController extends AbstractController
{
	...
	public String selectLanguage(@RequestParam("code") final String isoCode, final HttpServletRequest request)
	{
		...
		if (null != currentUserId && !currentUserId.equalsIgnoreCase("anonymous")
				&& ((boolean) sessionService.getAttribute("isSalesRepUserLoggedIn")))
		{*/
			final String previousUrl = request.getHeader("referer");
			if (null != previousUrl && !previousUrl.isEmpty())
			{
				return REDIRECT_PREFIX + previousUrl;
			}
		/* } */
