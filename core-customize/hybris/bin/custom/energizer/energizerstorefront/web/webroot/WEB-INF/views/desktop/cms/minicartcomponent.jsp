<%@ page trimDirectiveWhitespaces="true"%>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@ taglib prefix="ycommerce" uri="http://hybris.com/tld/ycommercetags" %>
<%@ taglib prefix="template" tagdir="/WEB-INF/tags/desktop/template"%>
<%@ taglib prefix="cart" tagdir="/WEB-INF/tags/desktop/cart"%>
<%@ taglib prefix="format" tagdir="/WEB-INF/tags/shared/format"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="theme" tagdir="/WEB-INF/tags/shared/theme"%>


<c:url value="/cart/miniCart/${totalDisplay}" var="refreshMiniCartUrl"/>
<c:url value="/cart/rollover/${component.uid}" var="rolloverPopupUrl"/>
<c:url value="/cart" var="cartUrl"/>
<%-- <spring:theme code="breadcrumb.cart" /> --%>
<c:set var="currencyISO" value="${totalPrice.currencyIso}" /> 

<!-- Added Code changes for WeSell Implementation - START -->
<c:choose>
	<c:when test="${not isSalesRepUserLogin}">
		<spring:theme code="breadcrumb.cart" />
		<a href="${cartUrl}" class="minicart">
	</c:when>
	<c:otherwise>
		<a href="${cartUrl}" style="text-transform: capitalize;"><spring:theme code="breadcrumb.cart" /></a>
		<a href="${cartUrl}" class="minicart" style="text-transform: uppercase !important;">
	</c:otherwise>
</c:choose>
<!-- Added Code changes for WeSell Implementation - END -->
	${component.title}   
	<ycommerce:testId code="miniCart_items_label">
		<span class="count">${totalItems}</span>
		<c:choose>
			<c:when test="${isSalesRepUserLogin}">
				<c:if test="${gotPriceFromSAP}" >
					<span class="price">
						<c:if test="${totalDisplay == 'TOTAL'  and empty showTaxForSalesRep}">
							<format:price priceData="${totalPrice}"  displayOnlyValue="true" currencyISO="${currencyISO}" totals="true"/> 
						</c:if>
						<c:if test="${totalDisplay == 'TOTAL' and not empty showTaxForSalesRep and showTaxForSalesRep}">
							<format:price priceData="${totalPriceWithTax}"  displayOnlyValue="true" currencyISO="${currencyISO}" totals="true"/>
						</c:if>
						<c:if test="${totalDisplay == 'SUBTOTAL'}">
							<format:price priceData="${subTotal}" displayOnlyValue="true" currencyISO="${currencyISO}" totals="true"/>
						</c:if>
						<c:if test="${totalDisplay == 'TOTAL_WITHOUT_DELIVERY'}">
							<format:price priceData="${totalNoDelivery}" displayOnlyValue="true" currencyISO="${currencyISO}" totals="true"/>
						</c:if>
					</span>
				</c:if>
			</c:when>
			<c:otherwise>
				<span class="price">
					<c:if test="${totalDisplay == 'TOTAL'}">
						<format:price priceData="${totalPrice}"/>
					</c:if>
					<c:if test="${totalDisplay == 'SUBTOTAL'}">
						<format:price priceData="${subTotal}"/>
					</c:if>
					<c:if test="${totalDisplay == 'TOTAL_WITHOUT_DELIVERY'}">
						<format:price priceData="${totalNoDelivery}"/>
					</c:if>
				</span>
			</c:otherwise>
		</c:choose>
	</ycommerce:testId>
</a> 


<div id="miniCartLayer" class="miniCartPopup" data-refreshMiniCartUrl="${refreshMiniCartUrl}/?"  data-rolloverPopupUrl="${rolloverPopupUrl}"></div>


