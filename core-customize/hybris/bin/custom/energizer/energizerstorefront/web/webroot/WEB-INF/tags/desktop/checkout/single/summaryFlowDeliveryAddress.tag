...
<div class="summarySection ${summarySectionClass} summaryDeliveryAddress ${summaryDeliveryAddressClass} ${deliveryAddressOk ? ' complete' : ''}" 
			...
			<div class="headline"></span><spring:theme code="checkout.summary.deliveryAddress.header" htmlEscape="false"/><span class="mandatory"><img width="5" height="6" alt="Required" title="Required" src="/_ui/desktop/common/images/mandatory.gif"></span></div>
				<ul>
					{{if data.deliveryAddress}}
						...
						<li>{{= data.deliveryAddress.erpAddressId}} - {{= data.deliveryAddress.companyName}}</li>
					...
					<div class="addressEntry  {{if defaultAddress}} default {{/if}}">
						...
						<div class="left edit_add_area">					
							<ul>
								...
								<li>{{= erpAddressId}} - {{= companyName}}</li>
								<li>{{= line1}}</li>
								<li>{{= line2}}</li>
								<li>{{= town}}</li>
								{{if region}}
